    ...
    render() {
        ...
        return (
            <div className='trivia-holder'>
                <img onClick={() => this.setState({ settings_open: !this.state.settings_open })} className='menu-button' src={require('./images/menu-white-48dp.svg')} alt='Settings' />
                {!this.state.settings_open ?
                    <div>
                        <div className='trivia-text'>
                            <div className={'difficulty-text ' + this.state.difficulty}>{this.state.difficulty}</div>
                            <div className="row">
                        ...
                        <div className='button-holder'>
                            {!this.state.reveal ?
                                this.state.show_choices ? <span></span> : <button onClick={() => this.setState({ reveal: true, next_disabled: false, show_choices: true })}>Show Answer</button>
                                :
                    ...
                    <div style={{ padding: '2vh' }}>
                        <span onClick={() => this.setState({ settings_open: false })}>Back</span> 
                        <h3>Settings</h3>
                        ...
                        <h3>Categories</h3>
                        <button onClick={()=> this.selectAllCategories()}>Toggle All</button>
                        {this.state.categories.map((category, index) => {
                            return (
                                <div key={index} className='category-item' onClick={()=>this.categoryChanged(category)}>
                                    {category.name}
                                    <Switch
                                        onChange={()=>null}
                                        checked={this.state.selected_categories.indexOf(category) !== -1}
                                    />
                                </div>
                            )
                        })}
